...
public class SplineController : MonoBehaviour {
	...
	bool FindSplineVertex(Spline s, Vector3 a, Vector3 b, SplineNode guess, out SplineNode vert, out Vector3 position) {
		...
		if(s != null) {
			...
			UtilitySpline.Vector3Pair points;
			while(countdown != null || countup != null) {
				if(countup.next != null) {
					...
					//#endif
					points = UtilitySpline.Dist3DSegToSeg(a, b, countup.transform.position, countup.next.transform.position);
				...
				if(countdown.previous != null) {
					...
					//#endif
					points = UtilitySpline.Dist3DSegToSeg(a, b, countdown.transform.position, countdown.previous.transform.position);
					if((points.a - points.b).magnitude < snapDistance) {
